...
class PianoVisualizer:
    def __init__(self, width=1600, height=500):
        pygame.init()
        pygame.midi.init()
        ...
        self.white_key_width = 60  # Increased from 30
        ...
        self.total_keys = 36  # Reduced from 88
        self.first_note = 48  # MIDI note number for C3 (instead of A0 which is 21)

        # Increased font sizes for better visibility
        self.font = pygame.font.SysFont("Arial", 24)  # Increased from 22
        self.key_font = pygame.font.SysFont("Arial", 18)  # Increased from 14
    ...
    def count_white_keys(self, start_note, end_note):
        """Count the number of white keys between two MIDI notes."""
        return len([n for n in range(start_note, end_note) if not self.is_black_key(n)])
    ...
    def get_key_position(self, note):
        ...
        if relative_note < 0 or relative_note >= self.total_keys:
            ...
            return self.piano_start_x + (
                self.count_white_keys(self.first_note, note) * self.white_key_width
            )
